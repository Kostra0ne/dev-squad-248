...
if (hacker1 === hacker2) {
  ...
  for (let i = 0; i < hacker1.length; i += 1) {
    if (hacker1[i].localeCompare(hacker2[i]) === 0) continue;
    else if (i >= hacker2.length) break;
    else if (hacker1[i].localeCompare(hacker2[i]) < 0) first = hacker1;
    else first = hacker2;
  }
  if (hacker1 === first) console.log("The driver's name goes first.");
  else console.log("Yo, the navigator goes first definitely");
}

console.log("lexico order", first);
